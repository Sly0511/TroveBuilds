...
class LocalizationManager:
    ...
    def update_all_translations(self):
        english = json.load(Path(get('load_path')[0]+"/en_US.json").open(encoding='utf'))
        for translation in self.get_all_translations():
            ...
            if before != translation_data:
                LocalizationLogger.debug(f'Updated translation file: {translation.stem}')
        ...
        for key, value in original.items():
            if key not in new.keys():
                ...
                if isinstance(value, dict):
                    self.ensure_keys(original[key], new[key])
                elif isinstance(value, str) and new[key].startswith(f"# TRANSLATION ERROR: "):
                    new[key] = f"# TRANSLATION ERROR: {value}"
